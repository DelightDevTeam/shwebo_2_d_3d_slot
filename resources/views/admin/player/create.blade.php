      ...
      <div class="card">
        ...
        <div class="card-body">
          <form role="form" method="POST" class="text-start" action="{{ route('admin.player.store') }}">
            ...
            <div class="custom-form-group">
              <label for="title">Player Name <span class="text-danger">*</span></label>
              <input type="text" name="name" class="form-control" value="{{old('name')}}" placeholder="Enter Player Name">
              @error('player_name')
              <span class="text-danger d-block">*{{ $message }}</span>
              @enderror
            </div>
...
<script>
  ...
  function copy() {
    var username = $('#tphone').text();
    var password = $('#tpassword').text();
    var copy = "url : " + url + "\phone : " + phone + "\npw : " + password;
